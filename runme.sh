...
readonly PROJECT_URL="https://raw.githubusercontent.com/PoPzQ/Wifi-Cashbot/${PROJECT_BRANCH}"
...
fn_setupProxy() {
    ...
    if [ "$PROXY_CONF" == 'false' ]; then
        while true; do
            ...
            colorprint "DEFAULT" "If you want to run multiple instances of WCB on same device you should setup the main instance with a proxy and then use mutiproxy function to setup all other replicas."
            read -r yn
            case $yn in
                [Yy]* )
                    clear
...
fn_checkDependencies(){
    clear
    colorprint "GREEN" "WIFI CASHBOT AUTOMATIC GUIDED SETUP v${SCRIPT_VERSION}"
    check_project_updates
